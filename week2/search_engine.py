...
def handle_oov_parenthesis(query):
    ...
    if query.startswith('('):  # ( word AND/OR word ) AND/OR word
        ...
        rewritten_rest = rewrite_query(re.sub(r'(and|or) ', '', rest))
        ...
        rewritten_start = rewrite_query(re.sub(r' (and|or)', '', start))
        rewritten_par = rewrite_query(parenthesis)
...
def rewrite_query(query):  # rewrite every token in the query
    query_parts = query.split()
    operators = [x for x in query_parts if x in d and x not in "()"]
    for i in range(len(query_parts)):
        ...
        if token not in terms and token not in d:
            ...
            if len(query_parts) == 1:
                ...
                # query = "unknown"
                return None
            elif all(o.lower() == "and" for o in operators):
                # All operators are "and" and there's an OOV-word and no parentheses = no matches
                return None
            elif i == 0:
                ...
                # query: two words, AND/OR, and NOT
                if i-2 > 0:
                ...
                prev_token = query_parts[i - 1].lower()
                next_token = query_parts[i + 1].lower()
                if prev_token == "or" and next_token == "or":
